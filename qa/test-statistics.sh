...
#   · 수입 대비 지출 — 수입 0 이면 null, 소수 1자리 HALF_UP (66.666 → 66.7)
...
#
#   2020-07  수입 3,000,000 / 지출 12,000          → 수입 대비 지출 0.4
#   2020-08  지출 5,000 만                         → 수입 0 → 수입 대비 지출 null, incomeList []
#   2020-10  수입 100,000 만 (+ 타인 지출 9,999)   → 지출 0 → 수입 대비 지출 0.0, expenseList []
#   2020-11  지출 50,000 → 삭제                    → 전부 0
#   2020-12  수입 500 / 지출 1,000                 → 수입 대비 지출 200.0, 잔액 -500
#   2021-03  수입 3,000 / 지출 1,500(대분류 직접) + 500(소분류)
#                                                  → 수입 대비 지출 66.7(HALF_UP), 봉투 2,000 = 직접 75.0 + 소분류 25.0
...
has 1-a "$(head4 500 1000 -500 200.0)"                         # 요약 4개, 수입 대비 지출 200.0
...
echo "───── 경계: 수입 0 (2020-08) — 수입 대비 지출 null, 수입 목록 빈 배열"
t 2 200 GET "$S?year=2020&month=8"
has 2-a "$(head4 0 5000 -5000 null)"
has 2-b '"incomeList":\[\]'
echo

echo "───── 경계: 지출 0 (2020-10) — 수입 대비 지출 0.0, 지출 목록 빈 배열, 타인 9,999 안 섞임"
...
echo "───── 소수점: 2020-07 수입 대비 지출 0.4 / 2021-03 66.7(HALF_UP) + 봉투 안 소분류 비율"
